...
from metric_fuc import predict2tag, get_embedding_matrix, F1ScoreCallback, ImprisonCallback, judger
...
model_name = 'cnn_model3'#sys.argv[1]
print(model_name)
...
hist = model.fit(x_train, y_train,
                 ...
                 verbose=1,
                 callbacks=[ImprisonCallback(), early_stopping, model_checkpoint, csv_logger],
                 # sample_weight=train_weight,
                 )
model.load_weights(bst_model_path)

predict = model.predict(x_test, batch_size=1024)
avgf1=judger(y_test,predict)
print("judger",j)
